...
def patch_runtime() -> None:
    ...
    old = 'scope = str(evidence_item.get("scope", "GLOBAL"))'
    new = 'scope = str(evidence_item.get("scope") or "")'
    if old not in text:
        raise RuntimeError('claim provenance scope anchor not found')
    text = text.replace(old, new, 1)

    old = '''                    source_meta["tenant_id"] = source_meta.get("tenant_id") or context.business_id
...
'''
    new = '''                    # Source provenance must come from the evidence itself, never from
...
'''
    if old not in text:
        raise RuntimeError('claim provenance backfill anchor not found')
    text = text.replace(old, new, 1)
    ...
    if start < 0 or end < 0:
        raise RuntimeError('performance block anchors not found')

    replacement = '''        # Mandatory internal Performance two-pass micro-workflow.
...
'''
    new = '''        authorization_status = audit_res.authorization_status
...
'''
    if old not in text:
        raise RuntimeError('final approval decision anchor not found')
    text = text.replace(old, new, 1)
...
'''
    if old not in text:
        raise RuntimeError('final approval output anchor not found')
    text = text.replace(old, new, 1)
    engine_path.write_text(text, encoding='utf-8')


def patch_governance() -> None:
    p = Path('governance/runtime_engine.py')
    text = p.read_text(encoding='utf-8')
    anchor = '        # 2. Permission Mode Evaluation\n'
    gate = '''        # Non-terminal claim authorization states never grant autonomous
...
'''
    if anchor not in text:
        raise RuntimeError('governance approval anchor not found')
    if 'Final claim authorization is not fully approved' not in text:
        text = text.replace(anchor, gate + anchor, 1)
    p.write_text(text, encoding='utf-8')


def migrate_main_regression_fixtures() -> None:
    perf = Path('tests/test_performance_two_pass_32.py')
    conditional = Path('tests/test_conditional_approval_no_deploy_55.py')
    for p in (perf, conditional, Path('tests/test_claim_provenance_no_backfill_33.py')):
        if not p.exists():
            raise RuntimeError(f'missing main regression test: {p}')
...
def main() -> int:
    ...
    if unresolved:
        raise RuntimeError(f'unexpected unresolved merge conflicts:\n{unresolved}')

    patch_runtime()
    patch_governance()
    migrate_main_regression_fixtures()

    run(sys.executable, '-m', 'py_compile', 'runtime/engine.py', 'governance/runtime_engine.py')
    run(
        sys.executable, '-m', 'unittest',
        'tests.test_performance_two_pass_32',
        'tests.test_claim_provenance_no_backfill_33',
        'tests.test_conditional_approval_no_deploy_55',
        'tests.test_runtime_checkpoint_hash_scope_integrity',
        'tests.test_approval_reference_scope_isolation',
        'tests.test_final_cmo_direct_publish_seam',
        'tests.test_final_cmo_deployment_registry_isolation',
        'tests.test_final_cmo_deployment_provenance',
    )

    Path('.github/workflows/candidate-final-main-reconcile-v1.yml').unlink(missing_ok=True)
    Path('scripts/_candidate_final_main_reconcile.py').unlink(missing_ok=True)
    run('git', 'add', '-A')
    run('git', 'commit', '-m', 'reconcile(main): preserve main hardening on final convergence')
    run('git', 'push', 'origin', 'HEAD:integration/final-main-reconciliation-v1')
    return 0
