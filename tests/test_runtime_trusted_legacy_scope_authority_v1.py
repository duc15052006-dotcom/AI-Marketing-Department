...
class RuntimeTrustedLegacyScopeAuthorityV1Tests(unittest.TestCase):
    ...
    def test_trusted_legacy_scope_alias_is_immutable_visible_to_compiler_and_spoof_resistant(self) -> None:
        ...
        grounded_memory_ids = {
            item.metadata.get("memory_id")
            for item in grounded.evidence_items
            if item.metadata.get("memory_id")
        }
        self.assertIn(trusted_doc.knowledge_id, grounded_knowledge_ids)
        self.assertNotIn(spoof_doc.knowledge_id, grounded_knowledge_ids)
        self.assertIn(trusted_memory.memory_id, grounded_memory_ids)
        self.assertNotIn(spoof_memory.memory_id, grounded_memory_ids)
