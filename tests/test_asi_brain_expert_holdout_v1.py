    ...
    def test_same_source_cannot_manufacture_corroboration(self):
        for agent in AGENTS:
            req = ClaimEvidenceRequest(
                ...
                agent_id=agent,
                evidence=[
                    evidence("e1", source="same-source", relation=EvidenceRelation.SUPPORTS, strength=EvidenceStrength.MODERATE),
                    evidence("e2", source="same-source", relation=EvidenceRelation.SUPPORTS, strength=EvidenceStrength.MODERATE),
                ],
            )
    ...
    def test_two_independent_observed_moderate_sources_do_support(self):
        for agent in AGENTS:
            req = ClaimEvidenceRequest(
                ...
                agent_id=agent,
                evidence=[
                    evidence("e1", source="s1", relation=EvidenceRelation.SUPPORTS, strength=EvidenceStrength.MODERATE),
                    evidence("e2", source="s2", relation=EvidenceRelation.SUPPORTS, strength=EvidenceStrength.MODERATE),
                ],
            )
    ...
    def test_derived_strong_evidence_cannot_establish_fact(self):
        for agent in AGENTS:
            req = ClaimEvidenceRequest(
                ...
                claim_id="P",
                agent_id=agent,
                evidence=[evidence("e1", source="derived", relation=EvidenceRelation.SUPPORTS, origin=EvidenceOrigin.DERIVED)],
            )
            self.assertEqual(assess_claim_evidence(req).verdict, ClaimVerdict.INSUFFICIENT)
...
class TestFiveASICollaborationAdversarial(unittest.TestCase):
    ...
    def test_four_peer_votes_cannot_upgrade_insufficient_proposal_evidence(self):
        ...
        result = evaluate_collaboration(
            collaboration(reviews, proposal_verdict=ClaimVerdict.INSUFFICIENT, proposal_refs=[], quorum=4)
        )
        self.assertEqual(result.disposition, CollaborationDisposition.INCONCLUSIVE)
    ...
    def test_missing_proposal_evidence_lineage_blocks_acceptance(self):
        result = evaluate_collaboration(
            collaboration([backed_review("s1", BrainAgentId.INTELLIGENCE)], proposal_refs=[])
        )
        self.assertEqual(result.disposition, CollaborationDisposition.INCONCLUSIVE)
