...
class _ControlledDateTime:
    current = _REAL_DATETIME(2001, 1, 1, tzinfo=timezone.utc)
...
class RuntimeMissionStoreExplicitClockAuthorityV1Tests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.base = _REAL_DATETIME(2001, 1, 1, tzinfo=timezone.utc)

        mission = MissionRecord(
            mission_id="MISSION-STORE-EXPLICIT-CLOCK-1",
            objective="Honor one explicit execution authority clock",
            business_id="BIZ-1",
            project_id="PROJ-1",
            user_id="USER-1",
        )
        mission.mark_ready(
            CommitmentRecord(
                commitment_id="COMMIT-STORE-EXPLICIT-CLOCK-1",
    ...
    def test_default_clock_still_rejects_expired_lease_after_transaction_read(self) -> None:
        lease = self._lease_and_activate()
        _ControlledDateTime.current = self.base + timedelta(seconds=11)
