...
class CapabilityRegistryPolicyImmutabilityAdversarialV2Tests(unittest.TestCase):
    @staticmethod
    ...
    def test_retained_registration_reference_cannot_downgrade_policy(self) -> None:
        registry = CapabilityRegistry()
        descriptor = self._critical_publish_descriptor("immutable_publish_input_v2")
        registry.register_capability(descriptor)

        self._downgrade(descriptor)

        self._assert_policy_still_authoritative(registry, "immutable_publish_input_v2")

    def test_get_return_value_cannot_downgrade_policy(self) -> None:
        registry = CapabilityRegistry()
        registry.register_capability(self._critical_publish_descriptor("immutable_publish_get_v2"))

        exposed = registry.get_capability("immutable_publish_get_v2")
        self.assertIsNotNone(exposed)
        assert exposed is not None
        self._downgrade(exposed)

        self._assert_policy_still_authoritative(registry, "immutable_publish_get_v2")

    def test_list_return_value_cannot_downgrade_policy(self) -> None:
        registry = CapabilityRegistry()
        registry.register_capability(self._critical_publish_descriptor("immutable_publish_list_v2"))

        exposed = next(
            item
            for item in registry.list_capabilities()
            if item.capability_id == "immutable_publish_list_v2"
        )
        self._downgrade(exposed)

        self._assert_policy_still_authoritative(registry, "immutable_publish_list_v2")

    def test_agent_list_return_value_cannot_downgrade_policy(self) -> None:
        registry = CapabilityRegistry()
        registry.register_capability(self._critical_publish_descriptor("immutable_publish_agent_list_v2"))

        exposed = next(
            item
            for item in registry.list_capabilities_for_agent("cmo")
            if item.capability_id == "immutable_publish_agent_list_v2"
        )
        self._downgrade(exposed)

        self._assert_policy_still_authoritative(registry, "immutable_publish_agent_list_v2")
