"""Adversarial truthfulness regression for production web-search zero-result handling v2.
...
class WebSearchZeroResultsTruthfulnessV2Tests(unittest.TestCase):
    def _execute_with_inner_result(self, inner_result: CapabilityResult):
        ...
        receipt = gateway.execute(
            ToolRequest(
                run_id="RUN-WEB-ZERO-RESULTS-V2",
                agent_id="intelligence",
                capability_id="web_search",
                parameters={"query": "market demand vietnam 2026"},
                business_id="BIZ-WEB-TRUTH-V2",
                project_id="PROJ-WEB-TRUTH-V2",
            )
        )
