...
from runtime.handoff import EpistemicType, build_epistemic_item, extract_handoff_payload
...
class StructuredScriptedGateway(UniversalModelGateway):
    ...
    def _bind_current_evidence_ref(cls, payload, prompt):
        """Bind fixtures only to observation evidence actually visible to the stage.

        A stage may see a fresh Live Tool Observation directly, or it may inherit
        an already-validated SOURCE_BACKED/VERIFIED upstream handoff reference.
        GENERATIVE/ACTION receipts never qualify merely because an EXEC-* token is
        ...
        inherited_refs = re.findall(
            r"\[(?:SOURCE_BACKED|VERIFIED)\][^\n]*?\(REFS:\s*(EXEC-[A-F0-9]{12})",
            prompt_upper,
        )
        refs = direct_refs + [ref for ref in inherited_refs if ref not in direct_refs]
...
class TestStructuredEpistemicHandoff(unittest.TestCase):
    ...
    def test_03_14_15_unknown_and_constraints_flow_to_intelligence(self):
        gw = StructuredScriptedGateway(replies={
            "cmo_initial": ("Delegation framed.", {"unknowns": ["Do we know the churn rate? No."]}),
        })
        rt = build_runtime(gw)
    ...
    def test_07_unsupported_fact_downgraded_not_verified(self):
        gw = StructuredScriptedGateway(replies={
            "intelligence": ("Prose.", {"facts": [{"text": "Rival is failing (no source)"}]}),
        })
        rt, ctx, final_out, artifact = run_pipeline(gw)
    ...
    def test_08_09_strategist_hypothesis_typed_for_creative(self):
        gw = StructuredScriptedGateway(replies={
            "content": ("Positioning prose.", {"hypotheses": [{"text": "GenZ pays +15% for eco packaging"}]}),
        })
        rt, ctx, final_out, artifact = run_pipeline(gw)
    ...
    def test_10_creative_claim_keeps_source_binding(self):
        gw = StructuredScriptedGateway(replies={
            "intelligence": ("Research prose.", {
                "facts": [{"text": "Lab observation available", "evidence_refs": [StructuredScriptedGateway.CURRENT_EVIDENCE_REF]}],
            }),
            "creative": ("Creative prose.", {
    ...
    def test_12_13_inconclusive_structural_and_not_overridable(self):
        gw = StructuredScriptedGateway(replies={"performance": ("RESULT: INCONCLUSIVE sample too small.", {})})
        rt, ctx, final_out, artifact = run_pipeline(gw)
    ...
    def test_16_17_handoff_survives_checkpoint_and_artifact(self):
        ...
        self.assertEqual(artifact.epistemic_handoffs, ctx.working_state["stage_handoffs"])
    ...
    def test_20_21_non_deployable_evidence_cannot_verify_fact(self):
        prov = {"TOOL-MOCK-1": {"epistemic_tier": "MOCK_OR_SANDBOX"}, "TOOL-GONE-1": None}
