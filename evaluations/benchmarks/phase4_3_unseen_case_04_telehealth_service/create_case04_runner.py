...
case_dir = Path(r"c:\AI-Marketing-Department\evaluations\benchmarks\phase4_3_unseen_case_04_telehealth_service")
...
# Prompt templates for C5
c5_prompt_builder = f"""You are a Principal Marketing Strategist and Medical Growth Director creating a complete, comprehensive, 28-deliverable Go-To-Market Strategy Proposal for CardioVital 360 (Case 04).
...
p_b5_p1 = build_candidate_b5_pass_1_prompt(facts, evidence, objective)
p_b5_p2 = build_candidate_b5_pass_2_prompt(facts, evidence, objective, "")
p_b5_p3 = build_candidate_b5_pass_3_prompt(facts, evidence, objective, "")
p_b5_p4 = build_candidate_b5_pass_4_prompt(facts, evidence, objective, "")

h_b5_p1 = hashlib.sha256(p_b5_p1.encode("utf-8")).hexdigest()
h_b5_p2 = hashlib.sha256(p_b5_p2.encode("utf-8")).hexdigest()
h_b5_p3 = hashlib.sha256(p_b5_p3.encode("utf-8")).hexdigest()
h_b5_p4 = hashlib.sha256(p_b5_p4.encode("utf-8")).hexdigest()

b5_full_hasher = hashlib.sha256()
b5_full_hasher.update(h_b5_p1.encode("utf-8"))
b5_full_hasher.update(h_b5_p2.encode("utf-8"))
b5_full_hasher.update(h_b5_p3.encode("utf-8"))
b5_full_hasher.update(h_b5_p4.encode("utf-8"))
h_b5 = b5_full_hasher.hexdigest()

h_c5 = hashlib.sha256(c5_prompt_builder.encode("utf-8")).hexdigest()
h_a5 = hashlib.sha256("FIVE_AGENT_BRAIN_V1_RC3_CASE_04_PROMPT_PIPELINE".encode("utf-8")).hexdigest()

# Protocol Manifest
case04_manifest = {
    "benchmark_id": "BENCHMARK-PHASE4-3-CASE04-TELEHEALTH-SERVICE",
    "case_id": "CASE_04_TELEHEALTH_REGULATED_SERVICE",
    "protocol_version": "v4_rc3",
    "brain_version": "FIVE_AGENT_BRAIN_V1_RC3",
    "brain_rc3_fingerprint": "b13f2f6c28007dd7a6f9e2a00175b9e87f59908ece647fb53ceba5186496354a",
    "benchmark_input_hash": "e8bd72e193bc1dd980ff0e667df3eff0f5e329248a2c9bf8cf19d78bb900b811",
    "prompt_hash_a5": h_a5,
    "prompt_hash_b5": h_b5,
    "prompt_hash_b5_passes": {
        "pass_1": h_b5_p1,
        "pass_2": h_b5_p2,
        "pass_3": h_b5_p3,
        "pass_4": h_b5_p4,
...
manifest_hasher.update(json.dumps(case04_manifest, sort_keys=True).encode("utf-8"))
protocol_fingerprint = manifest_hasher.hexdigest()
case04_manifest["protocol_fingerprint"] = protocol_fingerprint

(case_dir / "phase4_3c_16_benchmark_protocol.json").write_text(
    json.dumps(case04_manifest, indent=2), encoding="utf-8"
)
print("ACTIVE_CASE04_PROTOCOL_FINGERPRINT:", protocol_fingerprint)

# Write runner for Case 04
runner_code = '''"""Phase 4.3C.16: Final Unseen Case 04 Release Validation Runner.
...
def run_case04_release_validation() -> Dict[str, Any]:
    ...
    if run_dir_a5.exists():
        import shutil
        shutil.rmtree(run_dir_a5)
    ...
    if run_dir_b5.exists():
        import shutil
        shutil.rmtree(run_dir_b5)
    ...
    b5_actual_provider_tokens = sum(r["total_tokens"] for r in b5_telemetry_records)
    ...
    if run_dir_c5.exists():
        import shutil
        shutil.rmtree(run_dir_c5)
    for d in ["raw/request", "raw/response", "telemetry", "checkpoints"]:
        (run_dir_c5 / d).mkdir(parents=True, exist_ok=True)

    logger.info(f"Executing Candidate C5 (1 call): {run_id_c5}")
    from evaluations.benchmarks.phase4_3_unseen_case_04_telehealth_service.create_case04_runner import c5_prompt_builder
    p_prompt_c = c5_prompt_builder
...
(case_dir / "run_case04_validation.py").write_text(runner_code, encoding="utf-8")
print("Wrote Case 04 validation runner.")
